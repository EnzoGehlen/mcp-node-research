...
export const registerHostsResources = (server: McpServer): void => {
  // Get all connected hosts
  server.resource(
    "hosts-list",
    new ResourceTemplate("router://hosts", { list: undefined }),
    async () => {
  ...
  server.resource(
    "host-detail",
    new ResourceTemplate("router://hosts/{deviceId}", { list: undefined }),
    async (uri, { deviceId }) => {
  ...
  server.resource(
    "hosts-by-network",
    new ResourceTemplate("router://networks/{networkId}/hosts", { list: undefined }),
    async (uri, { networkId }) => {
      console.log(`[RESOURCE CALL] hosts-by-network - Listing hosts on network: ${networkId}`);
      const networkHosts = hostsData.devices.filter(
        d => d.connectionType === "wifi" && d.network === networkId
      );
